...
use Cypretex\Chat\Chat;

class Conversation extends \Cypretex\Chat\BaseModel {

    protected $table = 'mc_conversations';
    protected $fillable = ['data', 'id'];
    protected $casts = [
        'data' => 'array',
    ];
    ...
    public function last_message() {
        return $this->hasOne(Message::class)->where('type', '!=', 'text_bot')->orderBy('mc_messages.created_at', 'desc')->with('sender');
    }
    ...
    public function start($participants, $id = null, $data = []) {
        $conversation = $this->create(['id' => $id, 'data' => $data]);
        if ($participants) {
    ...
    private function getConversationMessages($user, $paginationParams, $deleted) {
        ...
        $messages = $messages->orderBy('mc_messages.created_at', $paginationParams['sorting'])
                ->paginate(
            ...
            'mc_messages.*',
                ], $paginationParams['pageName'], $paginationParams['page']
        );
