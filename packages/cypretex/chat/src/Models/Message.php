...
use Cypretex\Chat\Eventing\EventGenerator;
use Cypretex\Chat\Eventing\MessageWasSent;
use Cypretex\Chat\Models\Conversation;
use Cypretex\Chat\Models\MessageNotification;

class Message extends \Cypretex\Chat\BaseModel {

    use EventGenerator;

    protected $fillable = ['body', 'user_id', 'type', 'id', 'sent_at'];
     ...
     * @return Message
     */
    public function send(Conversation $conversation, $body, $userId, $id = null, $type = 'text', $sent_at = 0) {
        $message = $conversation->messages()->create([
            'id' => $id,
            'body' => $body,
            'user_id' => $userId,
            'type' => $type,
            'sent_at' => $sent_at > 0 ? $sent_at : time(),
        ]);
